...
export function LoBChart({ data }: LoBChartProps) {
      ...
      <CardContent>
        ...
        {selectedLoB && (
          <div className="mt-4 p-4 bg-muted rounded-lg">
            <h4 className="font-semibold text-sm mb-2">Selected LoB: {selectedLoB.name}</h4>
            <div className="grid grid-cols-3 gap-4 text-sm">
              <div>
                <span className="text-muted-foreground">Value:</span>
                <p className="font-medium">{formatBillion(selectedLoB.value)}</p>
              </div>
              <div>
                <span className="text-muted-foreground">Deals:</span>
                <p className="font-medium">{selectedLoB.count}</p>
              </div>
              <div>
                <span className="text-muted-foreground">Percentage:</span>
                <p className="font-medium">{selectedLoB.percentage.toFixed(1)}%</p>
              </div>
            </div>
          </div>
        )}
